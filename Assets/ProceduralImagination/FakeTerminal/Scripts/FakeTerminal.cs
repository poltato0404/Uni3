...
public class FakeTerminal : MonoBehaviour
{
    ...
    private void TerminalInput()
    {
        ...
        int lastIndex = KBmng.textBox.text.Length - 1;
        char c = KBmng.textBox.text[lastIndex];
        KBmng.pressed = false;
        {
            if (KBmng.backspace) //   Has backspace/delete been pressed?
            {
                KBmng.backspace = false;
            ...
            else if (KBmng.Return) // Has enter/return been pressed?
            {
                KBmng.Return = false;
                ...
                else
                {
                    ...
                    if(outputText[actualLine].Replace("" + cursor_Char, "").Length > lineIntro.Length)
                    {
                        ...
                        // ADD HERE NEW CUSTOM COMMANDS
                            ...
                            // [...]
                            //
                             foreach(string inputs_Return in inputs_Return)
                             {
                                 if(outputText[actualLine].Length >= lineIntro.Length)
                                 {
                                     if(outputText[actualLine].Replace("" + cursor_Char, "").Substring(lineIntro.Length) == inputs_Return)
                                     {
                                         printError = false;
                             ...
                             foreach(string inputs_L1M1 in inputs_L1M1)
                             {
                                 if(outputText[actualLine].Length >= lineIntro.Length)
                                 {
                                     if(outputText[actualLine].Replace("" + cursor_Char, "").Substring(lineIntro.Length) == inputs_L1M1)
                                     {
                                         printError = false;
                             ...
                             foreach(string inputs_L1M3 in inputs_L1M3)
                             {
                                 if(outputText[actualLine].Length >= lineIntro.Length)
                                 {
                                     if(outputText[actualLine].Replace("" + cursor_Char, "").Substring(lineIntro.Length) == inputs_L1M3)
                                     {
                                         printError = false;
                             ...
                             foreach(string inputs_nextLVL in inputs_nextLVL)
                             {
                                 if(outputText[actualLine].Length >= lineIntro.Length)
                                 {
                                     if(outputText[actualLine].Replace("" + cursor_Char, "").Substring(lineIntro.Length) == inputs_nextLVL)
                                     {
                                         printError = false;
                             ...
                             foreach(string inputs_L1M2 in inputs_L1M2)
                             {
                                 if(outputText[actualLine].Length >= lineIntro.Length)
                                 {
                                     if(outputText[actualLine].Replace("" + cursor_Char, "").Substring(lineIntro.Length) == inputs_L1M2)
                                     {
                                         printError = false;
